 ...
 * then this function inserts (ARMv7-M):
 * 	generateTest instructions -> only if necessary
 * 	AND r11, #<M1valueBasicBlock> -> only if necessary
 * 	EOR r11, #<M2valueBasicBlock> 
 * 
 * then this function inserts (ARMv6-M):
 * 	generateTest instructions -> only if necessary
 *  MOV r7, #<M1valueBasicBlock> 	-> only if necessary
 * 	AND r7, r5	 					-> only if necessary 
 *  MOV r7, #<M2valueBasicBlock>
 * 	EOR r7, r5
 */
void YACCA_Fast::insertEnd(unsigned int idBB, basic_block bb, rtx_insn* codeLabel){
	if( !(InstrType::isExitBlock(bb) && nrOfOrigInstr[idBB] == 1 ) ) {
		...
		switch(ARM_ISA::getISAtarget(arm_cpu_option)){
			...
			{
				if(M1Values[idBB] != -1){
					prev = AsmGen::emitMovRegInt(regsToUse[1], M1Values[idBB], prev, bb, true);
					prev = AsmGen::emitAndRegReg(regsToUse[0], regsToUse[1], prev, bb, true);
				}
				prev = AsmGen::emitMovRegInt(regsToUse[1], M2Values[idBB], prev, bb, true);
				AsmGen::emitEorRegReg(regsToUse[0], regsToUse[1], prev, bb, true);
				break;
			}
		}		
	}
...
void YACCA_Fast::insertSetup(){
	...
	if( ARM_ISA::getISAtarget(arm_cpu_option) == ARMv7M){
		AsmGen::emitMovRegInt(regsToUse[2], 0, prev, bb, true); // Only ARMv7-M uses the 3rd register
	}	
}
...
rtx_insn* YACCA_Fast::generateTest(unsigned int idBB, basic_block bb, rtx_insn* codeLabel, rtx_insn* attachRTX, bool after){
	...
	if(predecessors.size() == 1){
		...
		switch(ARM_ISA::getISAtarget(arm_cpu_option)){
			...
			case ARMv6M:
			default:
				prev = AsmGen::emitBneRegReg(regsToUse[0], regsToUse[1], codeLabel, prev, bb, true);
				break;
		}
		...
		switch(ARM_ISA::getISAtarget(arm_cpu_option)){
			...
			default:
			{
				rtx_insn* contLabel = AsmGen::emitCodeLabel(get_max_uid() +1, prev, bb, after);
				prev = contLabel;	// update prev to correctly place the chain
				after = false;		// first instruction of the chain must come before the continue label! 
				for(it = predecessors.begin(); it != std::prev(predecessors.end() ); it++){		// loop over all predecessors, except the last one
					prev = AsmGen::emitMovRegInt(regsToUse[1], *it, prev, bb, after);		// also update chain point!
					after = true;	// all next instructions in the chain come after this one!
					prev = AsmGen::emitBeqRegReg(regsToUse[0], regsToUse[1], contLabel, prev, bb, true);	// values match, so skip rest and execute from contLabel onwards
				} 
				// Handle the last predecessor
				it = std::prev(predecessors.end());
				prev = AsmGen::emitMovRegInt(regsToUse[1], *it, prev, bb, true);
				AsmGen::emitBneRegReg(regsToUse[0], regsToUse[1], codeLabel, prev, bb, true);	// last predecessor, so mismatch indicates error! hence error label usage!
				prev = contLabel;	// point prev back to the label, so the remainder of insertEnd is inserted correctly.
				break;
			}
		}		
	}
