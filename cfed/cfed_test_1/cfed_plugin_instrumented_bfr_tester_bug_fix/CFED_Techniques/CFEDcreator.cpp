...
void CFEDcreator::implementTechnique(const char *technique, bool intraBlockDet, unsigned int selectiveLevel){
	...
	if(!strcmp(technique, "RACFED")){
		...
		if(ARM_ISA::getISAtarget(arm_cpu_option) == ARMv6M){
			throw "ARMv6M does not support hardware division and hence ECCA cannot be implementend!\n";
		}
		else{
		...
		if(ARM_ISA::getISAtarget(arm_cpu_option) == ARMv6M){
			throw "ARMv6M does not support hardware division and hence YACCA cannot be implementend!\n\tYACCA_Fast presents an alternative\n";
		}
		else{
