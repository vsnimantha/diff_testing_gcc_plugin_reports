...
void SIED::insertBegin(unsigned int idBB, basic_block bb, rtx_insn* codeLabel, rtx_insn* attachBefore){
	...
	if (intraDet){
		if(idBB != 0){
			switch(ARM_ISA::getISAtarget(arm_cpu_option)){
				...
				case ARMv6M:
				default:
					prev = AsmGen::emitBneRegInt(regsToUse[0], 0, codeLabel, prev, bb, false);
					break;
			}
		}
	}
	...
	switch(ARM_ISA::getISAtarget(arm_cpu_option)){
		case ARMv7M:	
			prev = AsmGen::emitCmpRegReg(regsToUse[1], regsToUse[2], prev, bb, true);
			AsmGen::emitBne(codeLabel, prev, bb, true);
			break;
		case ARMv6M:
		default:
			AsmGen::emitBneRegReg(regsToUse[1], regsToUse[2], codeLabel, prev, bb, true);
			break;
	}
}
		...
		if(JUMP_P(last)){
			if(InstrType::isCondJump(last)){
				switch(ARM_ISA::getISAtarget(arm_cpu_option)){
					...
					default:
					{	
						AsmGen::emitMovRegInt(regsToUse[1], 1, last, bb, false);							// before the branch (last)
						AsmGen::emitMovRegInt(regsToUse[2], branchSigs[idBB].trueBranch, last, bb, false);	// before the branch (last)
						AsmGen::emitMovRegInt(regsToUse[1], 0, last, bb, true);								// after the branch (last)
						AsmGen::emitMovRegInt(regsToUse[2], branchSigs[idBB].falseBranch, last, bb, true);	// after the branch (last)
						break;
					}
				}
			}
