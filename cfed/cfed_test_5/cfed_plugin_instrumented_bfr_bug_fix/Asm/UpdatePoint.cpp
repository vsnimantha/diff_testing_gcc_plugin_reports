...
rtx_insn* UpdatePoint::lastRealSafeINSN(basic_block bb){
	...
	if(InstrType::isReturn(lastInsn)){
		...
		switch(ARM_ISA::getISAtarget(arm_cpu_option)){
			case ARMv7M:
				if(JUMP_P(lastInsn) && !InstrType::isCondJump(lastInsn)){
					lastInsn = PREV_INSN(lastInsn);
				}
				...
				break;
			case ARMv6M:	// ARMv6-M does not allow freestanding CMP. CMP+B<cond> is always 1 instruction, so can simplify this search.
			default:
				if(JUMP_P(lastInsn)){
					lastInsn = PREV_INSN(lastInsn);
				}
