 ...
 * Emits: EOR reg, reg
 */
rtx_insn* AsmGen::emitEorRegReg(unsigned int destReg, unsigned int operand1, unsigned int operand2, rtx_insn* attachRtx, basic_block bb, bool after){
	...
	rtx xorRtx = gen_rtx_XOR(SImode, operand1Rtx, operand2Rtx);
	rtx set = gen_movsi(destRegRtx, xorRtx);
	rtx_insn* insn = emitInsn(set, attachRtx, bb, after);
	return insn;
}
...
/**
 * Emits: .codeLabel
 * TODO why does this method require insnID and bb?
 */
 ...
 * Emits: BHI .codelabel
 */
rtx_insn* AsmGen::emitBhi(rtx_insn* codeLabel, rtx_insn* attachRtx, basic_block bb, bool after){
	rtx regCC = gen_rtx_REG(CCmode, CC_REGNUM);
	rtx constInt = gen_rtx_CONST_INT(VOIDmode, 0);
	rtx hi = gen_rtx_GT(CCmode, regCC, constInt);
	return emitInsn(gen_arm_cond_branch(codeLabel, hi, regCC), attachRtx, bb, after);
}

/**
 * Emits: BLO .codelabel
 */
rtx_insn* AsmGen::emitBlo(rtx_insn* codeLabel, rtx_insn* attachRtx, basic_block bb, bool after){
	rtx regCC = gen_rtx_REG(CCmode, CC_REGNUM);
	rtx constInt = gen_rtx_CONST_INT(VOIDmode, 0);
	rtx hi = gen_rtx_LT(CCmode, regCC, constInt);
	return emitInsn(gen_arm_cond_branch(codeLabel, hi, regCC), attachRtx, bb, after);
}
 ...
 * Emits: B .codeLabel
 */
rtx_insn* AsmGen::emitB(rtx_insn* codeLabel, rtx_insn* attachRtx, basic_block bb, bool after) {
	// rtx regCC = gen_rtx_REG(CCmode, CC_REGNUM);
	// rtx constInt = gen_rtx_CONST_INT(VOIDmode, 0);
	rtx_insn* insn = emitInsn(gen_jump(codeLabel), attachRtx, bb, after);
	return insn;
}
...
/**
 * Emits: BL <functionName>
 */
rtx_insn* AsmGen::emitBl(const char* functionName, rtx_insn* attachRtx, bool after) {
	rtvec vec = rtvec_alloc(3);
	rtx constInt = gen_rtx_CONST_INT(VOIDmode, 0);
	// Create call
	rtx symbol = gen_rtx_SYMBOL_REF(SImode, functionName);
	...
	// Emit
	if(after) {
		return emit_call_insn_after_noloc(par, attachRtx);
	} else {
		return emit_call_insn_before_noloc(par, attachRtx);
	}	
}
...
rtx_insn* AsmGen::emitCall(rtx_insn* codeLabel){
	emitBl("CFED_Detected", codeLabel, true);
}
