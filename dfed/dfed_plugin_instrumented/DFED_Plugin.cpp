 ...
 * about the pass.
 */
const struct pass_data DFED_PLUGIN_pass_data =
{
		.type = RTL_PASS,
		.name = "myPlugin",
 ...
 */
DFED_PLUGIN::DFED_PLUGIN(gcc::context *ctxt, struct plugin_argument *arguments, int argcounter)
		: rtl_opt_pass(DFED_PLUGIN_pass_data, ctxt)
{
 ...
 */
bool DFED_PLUGIN::gate(function *fun)
{
 ...
 */
unsigned int DFED_PLUGIN::execute(function *fun)
{
	...
	Printer::printXML((char *)"CFG.xml");			// Print the CFG in xml format
 ...
 * args array, matching the given key
...
const char* DFED_PLUGIN::findArgumentValue(const char* key){
	DFED_PLUGIN::findArgumentValue(key, args, argc);
}
 ...
 * @param key The key of the argument to get the value for
 * @param args The array containing the plugin arguments
 ...
 */
bool DFED_PLUGIN::isAllowedToRun(const char* funName){
	try{
		...
		else if( (!strcmp(f, funName)) ){
			return true;
		}
		else{
			char fCopy[strlen(f)+1];					// Create non const copy of f
			strcpy(fCopy, f);
			char* fCopyParts = strtok(fCopy, ",");		// split the copy into parts, based on the ',' char
			bool foundFunName = false;
			while(fCopyParts != NULL){					// Loop over all parts
				if (!strcmp(fCopyParts, funName)){		// Search if funName is in current part
					foundFunName = true;
					break;
				}
				fCopyParts = strtok(NULL, ",");
			}
			return foundFunName;
		}
	}
...
void DFED_PLUGIN::implementDetectionTechnique(){
	try{
		// TODO Evaluate if extra arguments are necessary
		const char* technique = findArgumentValue("techniqueSpecific");
		DFEDcreator* dfedCreator = new DFEDcreator();
		dfedCreator->implementTechnique(technique, args, argc);
	} catch (const char* e){
		printf("\x1b[91mDFE Detection Technique was not implemented:\x1b[0m\n\t%s\n", e);
	}
}
