...
FDEC::FDEC(ARM_ISA* isa, map<unsigned char, unsigned char> regMap)
	:GeneralDFED(isa, regMap){
	popStrings.reserve(6);
}

void FDEC::implementDuplicationComparison(rtx_insn* codeLabel){
	implementDuplication();
	implementComparison(codeLabel);
}

void FDEC::implementDuplication(){
	basic_block bb;
...
void FDEC::implementComparison(rtx_insn* codeLabel){
	basic_block bb;
	FOR_EACH_BB_FN(bb, cfun){
...
rtx_insn* FDEC::handlePushInstruction(rtx_insn* pushInsn, basic_block bb){
...
void FDEC::handlePopInstruction(rtx_insn* popInsn, basic_block bb){
	vector<unsigned char> usedRegs;
...
void FDEC::findAllUsedRegisters(rtx expr, vector<unsigned char>* usedRegs){
