...
FDSC::FDSC(ARM_ISA* isa, map<unsigned char, unsigned char> regMap)
	: GeneralDFED(isa, regMap){
	popStrings.reserve(6);
}

void FDSC::implementDuplicationComparison(rtx_insn* codeLabel){
	basic_block bb;
	FOR_EACH_BB_FN(bb, cfun){
		rtx_insn* lastRealSafeInsn = UpdatePoint::lastRealSafeINSN(bb);
		rtx_insn* lastRealInsn = UpdatePoint::lastRealINSN(bb);
		rtx_insn* insn;
		bool isVulblock = isVulnerableBlock(bb);
			...
			if(NONDEBUG_INSN_P(insn) && !JUMP_P(insn) && !CALL_P(insn) && !InstrType::isUse(insn)
				...
				if(!InstrType::isCompare(insn) && !InstrType::isStore(insn) && !InstrType::isPush(insn) && !InstrType::isPop(insn)){
					...
					if(InstrType::isCondExec(duplicate)){	// No comparison necessary if conditional execution
						...
						if(lastRealSafeInsnBehind(duplicate, lastRealInsn, lastRealSafeInsn) && isVulblock){
					...
					else if (isVulblock){	// Always insert a comparison instruction in this case
						insn = insertSingleComparisonAfter(insn, duplicate, codeLabel, bb);
					}
					else{
						insn = duplicate;
				...
				else if(isVulblock && (InstrType::isCompare(insn) || InstrType::isStore(insn) ) ){		// A compare or store instruction needs no duplicate, only CMP for its used register(s)
...
bool FDSC::isVulnerableBlock(basic_block bb){
	return bb->preds->length() > 1;
}

bool FDSC::lastRealSafeInsnBehind(rtx_insn* duplicate, rtx_insn* lastRealInsn, rtx_insn* lastRealSafeInsn){
...
rtx_insn* FDSC::insertSingleComparisonAfter(rtx_insn* origInsn, rtx_insn* attachRtx, rtx_insn* codeLabel, basic_block bb){
...
void FDSC::insertMultipleComparisonInstructionsBefore(rtx_insn* origInsn, rtx_insn* codeLabel, basic_block bb){
	vector<unsigned char> usedRegs;
	findAllUsedRegisters(origInsn, &usedRegs);							// 1) Find all used registers
...
rtx_insn* FDSC::handlePushInstruction(rtx_insn* pushInsn, basic_block bb){
	vector<unsigned char> usedRegs;
...
void FDSC::handlePopInstruction(rtx_insn* popInsn, basic_block bb){
	vector<unsigned char> usedRegs;
...
void FDSC::findAllUsedRegisters(rtx expr, vector<unsigned char>* usedRegs){
