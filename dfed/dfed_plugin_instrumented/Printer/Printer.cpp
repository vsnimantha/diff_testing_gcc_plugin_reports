	...
	if (fp != NULL){
		...
		FOR_ALL_BB_FN(bb, cfun){					// Loop over all Basic Blocks in the function, cfun = current function
			...
			FOR_BB_INSNS(bb, insn){
				// Loop over all rtx statements in the Basic Block
...
/**
 * Function to build the correct *.xml file that is
 * Can be used in the visualizer.
 */
void Printer::printXML( char* fileName){
        char tempName[512];
        createFileName(tempName, fileName);
        FILE *fp = fopen(tempName, "w");
        if (fp != NULL){
                fprintf(fp, "<?xml version=\"1.0\"?>\r\n");
                fprintf(fp,"<visualization>\r\n");
                //BasicBlock
                basic_block bb;
                FOR_EACH_BB_FN(bb, cfun){
                        fprintf(fp,"<BasicBlock id=\"%d\">\r\n", (bb->index)-2);
                        //insn's
                        rtx_insn* insn;
                        FOR_BB_INSNS(bb, insn){
                                if(NONDEBUG_INSN_P(insn)){
                                        fprintf(fp, "<instr id=\"%d\">\r\n", XINT(insn,2));
                        ...
                        edge_iterator ei;
                        FOR_EACH_EDGE(e, ei, bb->succs){
                                fprintf(fp, "<edge>%d</edge>\r\n", (e->dest)->index - 2);
                        }
                        fprintf(fp, "</BasicBlock>\r\n");
                }
                fprintf(fp, "</visualization>");
                fclose(fp);
        }
