 ...
 * Emits STMDB <stackPointer>!, {<reglist>} with each necessary register contained in <reglist>
 * Uses the pushPopStrings variable and the emitAsmInput method of AsmGen class.
 */
rtx_insn *ARMv7M_Functions::insertPush(vector<unsigned char> &regs, unsigned char stackPointer, rtx_insn* attachRtx, basic_block bb, bool after){
	// Emit the push
	string push = "r" + to_string(regs[regs.size()-1]);
	for(int i = regs.size()-2; i > -1 ; i--){
		push += ", r" + to_string(regs[i]);
	}
	string fullPush = "STMDB r" + to_string(stackPointer) + "!, {" + push + "}";
	pushPopStrings.push_back(fullPush);
	return AsmGen::emitAsmInput(pushPopStrings[pushPopStrings.size()-1].c_str(), attachRtx, bb, after);
}

/**
 * Function to emit the necessary POP instruction
 * Emits LDMIA <stackPointer>!, {<reglist} with each necessary register contained in <reglist>
 * Uses the pushPopStrings variable and the emitAsmInput method of AsmGen class.
 */
rtx_insn *ARMv7M_Functions::insertPop(vector<unsigned char> &regs, unsigned char stackPointer, rtx_insn* attachRtx, basic_block bb, bool after){
	string pop = "r" + to_string(regs[regs.size()-1]);
	for(int i = regs.size()-2; i > -1 ; i--){
		pop += ", r" + to_string(regs[i]);
	}
	string popPush = "LDMIA r" + to_string(stackPointer) + "!, {" + pop + "}";
	pushPopStrings.push_back(popPush);
	return AsmGen::emitAsmInput(pushPopStrings[pushPopStrings.size()-1].c_str(), attachRtx, bb, false);
}
