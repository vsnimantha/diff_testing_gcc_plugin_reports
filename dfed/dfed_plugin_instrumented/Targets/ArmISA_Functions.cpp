...
vector<char> ARM_ISA::getRegsFromStr(const char* regsStr) {
	throw "ARM_ISA::getRegsFromStr(const char* regsStr) should be overridden";
}
 ...
 */
void ARM_ISA::insertPushPop(vector<unsigned char> regs){
 ...
 */
rtx_insn *ARM_ISA::insertPush(vector<unsigned char> &regs) {
	return insertPush(regs, this->stackPointer);
}

rtx_insn *ARM_ISA::insertPush(vector<unsigned char> &regs, unsigned char stackPointer) {
	return insertPush(regs, stackPointer, get_first_nonnote_insn(), BASIC_BLOCK_FOR_FN(cfun,2), false);
}
 ...
 * Uses the pushPopStrings variable and the emitAsmInput method of AsmGen class.
 */
rtx_insn *ARM_ISA::insertPop(vector<unsigned char> &regs, rtx_insn* last, basic_block bb){
	return insertPop(regs, this->stackPointer, last, bb);
}

rtx_insn *ARM_ISA::insertPop(vector<unsigned char> &regs, unsigned char stackPointer, rtx_insn* last, basic_block bb) {
	return insertPop(regs, stackPointer, last, bb, false);
}
